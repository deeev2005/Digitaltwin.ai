...
export const STATIONS: Station[] = [
  ...BODY_NAMES.map((n, i) => {
    const id = i + 1;
    const inst: Instrumentation = id === 13 ? "proxy" : "sensor";
    return make(
      id,
      n,
      "BODY",
      inst,
      inst === "sensor" ? SENSOR_SIGNALS : PROXY_SIGNALS,
      inst === "sensor" ? 0.94 : 0.71,
    );
  }),
  ...PAINT_NAMES.map((n, i) => {
    const id = i + 15;
    const inst: Instrumentation = id === 23 ? "proxy" : "sensor";
    return make(
      id,
      n,
      "PAINT",
