  ...
  return (
    <div className="relative overflow-x-auto select-none">
      <div className="min-w-[1240px] px-2 pt-2 pb-2">
          ...
          <div className="relative h-[155px] w-full">
            ...
            >
              ...
              {STATIONS.map((s, index) => {
                ...
                const isUnwired = s.id === 4 || s.id === 8 || s.id === 16 || s.id === 26 || s.id === 31 || s.id === 33;
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-x-6 gap-y-3 border-t border-border/60 pt-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-2.5 font-medium tracking-wide">
            <span className="text-[11px] uppercase tracking-wider text-muted-foreground/80 font-semibold">Presented by</span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-purple-500/20 via-indigo-500/20 to-blue-500/20 border border-purple-500/30 text-purple-300 shadow-[0_0_12px_rgba(168,85,247,0.15)]">
          ...
          <div className="flex items-center gap-3 text-xs font-medium">
            <span className="text-muted-foreground/70 font-mono text-[11px] uppercase tracking-wider">Team Members</span>
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <span className="flex items-center gap-2 text-foreground/90 bg-panel-raised/80 px-3 py-1 rounded-md border border-border/60 hover:border-purple-500/40 transition-colors shadow-xs">
        ...
        {/* Demo Disclaimer */}
        <div className="mt-2.5 pt-2 border-t border-border/30 text-center font-mono text-[10px] text-muted-foreground/70 leading-relaxed">
          * This demo is a simulated representation for illustration purposes and does not reflect actual proprietary plant telemetry. Multiple scenarios depicted across views may or may not be directly related to one another.
        </div>
      </div>
    </div>
  );
...
function StationNode({
  ...
  const isUnwired = station.id === 4 || station.id === 8 || station.id === 16 || station.id === 26 || station.id === 31 || station.id === 33;
